...
public abstract class BigMapIdPersistenceStore<V> extends BigMapPersistenceStore<String, V>
  ...
  public Optional<WorkflowInstance> findById(
      WorkflowDefinition definition, String workflowInstanceId) {
    return Optional.ofNullable(instances(definition).get(workflowInstanceId))
        .map(v -> unmarshall(definition, v));
  }
}
